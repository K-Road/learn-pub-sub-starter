...
func main() {
	...
	for {
		...
		case "spam":
			...
			if len(input) != 2 {
				fmt.Println("usage: spawn <number>")
				continue
			}
			spam, err := strconv.Atoi(input[1])
			if err != nil {
				continue
			}
			fmt.Print(spam)
			for i := 0; i < spam; i++ {
				...
				err = pubsub.PublishGob(publishCh, routing.ExchangePerilTopic, routing.GameLogSlug+"."+gameState.GetUsername(), routing.GameLog{
